...
def simplex(A,b,c,initial_point,print_details_flag=False):
    ...
    while True:
        if iter==10000:
            print("\nBREAKING AT THE 10000TH ITERATION TO AVOID INFINITE LOOPING!DEGENERATE. PROBLEM")
            exit()
...
def main():
    ...
    if min_b<0: # if there is a negative entry in b
        ...
        print("\nPhase 2 begins...")
    x=simplex(A,b,c,initial_point,True)
    print("Optimum value:",np.round(np.dot(c,x), 4),"\nSolution vector:",np.round(x, 4))
